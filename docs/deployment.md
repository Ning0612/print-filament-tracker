...
本文說明如何在 **Windows** 和 **macOS** 上安裝與執行 FilamentLedger。
...
FilamentLedger 提供預先打包的執行檔，**無需安裝 Python**：
...
前往 [GitHub Releases](https://github.com/Ning0612/filament-ledger/releases) 下載最新版，有兩種版本可選：

**方式一：MSI 安裝程式（推薦）**

1. 下載 `FilamentLedger-x.x.x.msi`
2. 雙擊執行，依照安裝精靈完成安裝：
   - **選擇功能**：可勾選是否建立「桌面捷徑」（預設勾選）
   - **完成畫面**：可勾選「立即啟動 FilamentLedger」（預設勾選）
3. 安裝後可從「開始選單」或桌面捷徑啟動，並在「設定 → 新增或移除程式」中管理

**方式二：免安裝 .exe**

1. 下載 `FilamentLedger.exe`
2. 將 `.exe` 移至您希望長期存放的目錄，例如：
   ```
   C:\Users\你的名字\FilamentLedger\FilamentLedger.exe
   ```
3. 雙擊 `FilamentLedger.exe` 啟動
...
```powershell
explorer "$env:LOCALAPPDATA\FilamentLedger"
```
...
**解除安裝（MSI 安裝版）**：
1. 右鍵系統托盤圖示 → 結束程式
2. 前往「設定」→「應用程式」→「已安裝的應用程式」，找到 FilamentLedger 後解除安裝
3. （可選）刪除資料目錄：`%LOCALAPPDATA%\FilamentLedger\`
...
1. 前往 [GitHub Releases](https://github.com/Ning0612/filament-ledger/releases) 下載最新版 `FilamentLedger.app.zip`
2. 解壓縮後，將 `FilamentLedger.app` 移至您希望存放的目錄（如 `~/Applications/`）
3. **首次開啟**（必須使用右鍵開啟以繞過 Gatekeeper）：
   - 右鍵點擊 `FilamentLedger.app`
   - 選擇「**開啟**」
   - 在出現的對話框中點擊「**開啟**」確認
...
若出現「FilamentLedger 無法開啟，因為開發者無法驗證」：
...
```
~/Library/Application Support/FilamentLedger/
├── .env              ← 設定檔（首次啟動時自動建立）
└── data/
    ├── tracker.db    ← SQLite 資料庫
    ├── covers/       ← 封面圖片
...
```bash
open ~/Library/Application\ Support/FilamentLedger
```
...
2. 將 `.app` 移至垃圾桶
3. （可選）刪除資料目錄：`~/Library/Application Support/FilamentLedger/`
...
```
# Windows
%LOCALAPPDATA%\FilamentLedger\.env

# macOS
~/Library/Application Support/FilamentLedger/.env
```
...
```powershell
# Windows
Get-Content "$env:LOCALAPPDATA\FilamentLedger\data\logs\app.log" -Wait -Tail 50
```

```bash
# macOS
tail -f ~/Library/Application\ Support/FilamentLedger/data/logs/app.log
```
...
```powershell
# Windows（輸出：dist\FilamentLedger.exe + dist\FilamentLedger-x.x.x.msi）
...
```bash
# macOS（輸出：dist/FilamentLedger.app）
