...
採用「編輯型工業工作室」語言：安靜、精準、可掃讀，像一本持續更新的材料帳本。**帳本語言集中在資料本身（表格規線、出入帳色語、印章式狀態、合計雙下劃線）**。

**v1.2 起，shell 本體採實體帳冊的「結構性擬真線索」**——皮革書脊、索引書籤側欄、書頁 leaf、絲帶書籤、裝訂溝與疊頁邊——但**仍不鋪任何整頁紙紋或規線背景**：質感來自邊緣、裝訂與陰影等結構線索，而非平鋪紋理（後者讀起來像廉價雜訊）。書頁本體維持乾淨暖紙色。
...
實體帳冊 shell 專用 token，light/dark 各一份定義於 `:root` 與 `[data-theme="dark"]`。皮革走深褐↔酒紅，燙金字走暖奶油。

| Token | 用途 |
|---|---|
| `--ui-book-spine` / `--ui-book-spine-2` / `--ui-book-spine-edge` | 皮革書脊三階漸層 |
| `--ui-book-spine-ink` | 書脊垂直燙金書名 |
| `--ui-book-tab-face` | 索引書籤底色 |
| `--ui-book-page-edge` / `--ui-book-page-shadow` | 疊頁邊與書頁陰影 |
| `--ui-book-ribbon` / `--ui-book-ribbon-2` | 絲帶書籤漸層 |
| `--ui-book-thread` | 裝訂針腳 |
| `--ui-book-spine-w` | 書脊 grid 欄寬（`:root` 44px，收縮覆寫 30px） |
...
```text
.app-shell（grid：spine | sidebar | main）
├── .app-spine         皮革書脊（grid 欄，預設 44px、收縮 30px；aria-hidden）
├── .app-sidebar       232px sticky sidebar（索引書籤側欄）
└── .app-main-shell    書頁 leaf（surface 底、右側圓角框、左緣裝訂溝）
    ├── .leaf-ribbon   絲帶書籤（absolute 裝飾，pointer-events:none）
    ├── .leaf-edge     疊頁邊＋右下摺角（absolute 裝飾）
    ├── .app-content
    └── .app-footer
```

導覽分為「總覽、材料、列印、洞察、系統」五個群組。現有 URL 與 `aria-current="page"` 判斷必須保留。導覽連結以 v1.2 索引書籤呈現：目前頁的 `[aria-current="page"]` tab 以 `--ui-surface` 齊側欄右緣、與書頁同色相連（側欄已移除 `border-right`，避開 `overflow-x:hidden` 裁切）。書脊、絲帶、疊頁在收縮模式縮化、在手機（≤820px）與兩處 `@media print` 全數隱藏；裝飾節點一律 `aria-hidden` 且 `pointer-events:none`，不佔 layout、不進 tab order。
...
每日細節頁（`analytics/day.html`，v1.2）以「一天一頁」的書頁語言呈現：eyebrow 標「西元年-月」分卷、`.day-pager` 提供「前一日／後一日」書本翻頁（連到最近一個 started-date 有資料日，永不 404；next 以本地今天為上限；無相鄰日時以非聚焦 `aria-disabled` 呈現、不留可點的 disabled anchor），`.month-summary` 顯示「本月至今」日結摘要（用量／次數／花費，口徑與每日卡片一致：月摘要＝Σ 每日）。成本列印頁底部加「製表戳記」腳註（`cost.generated_on`）。
...
- shell 書本化：新增皮革書脊（grid 欄）、索引書籤側欄（active tab 併入書頁）、書頁 leaf（裝訂溝＋右側框）、絲帶書籤與疊頁邊／摺角裝飾；質感走結構線索，仍不鋪整頁紙紋。
- 新增 `--ui-book-*` 皮革 token（light/dark）；書脊寬 `--ui-book-spine-w`（44px／收縮 30px）。
- 收縮模式、手機（≤820px）與兩處 `@media print` 皆對書本裝飾降級／隱藏；裝飾節點 `aria-hidden`＋`pointer-events:none`，不佔 layout。
- 每日細節頁加前後日翻頁（永不 404、next 以今天為上限、disabled 態不可聚焦）與「本月至今」日結摘要（月＝Σ 每日）；新增 `src/db/_analytics.py` 唯讀查詢與 `src.analytics.get_month_to_date_summary`。
- 共用 `ledger_eyebrow` macro；任務清單加本頁小計、成本列印頁加製表戳記。
